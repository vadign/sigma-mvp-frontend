...
function DataAndTopologyPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchTopology(selectedNetworkId), fetchLogs(selectedNetworkId)])
      .then(([topo, logList]) => {
        setTopology(topo);
        setLogs(logList);
        if (logList.length > 0) setSelectedLogId(logList[0].id);
        else {
  ...
  const getEdgeColor = (level?: number | null) => {
    if (level === 3) return '#ff4d4f';
    if (level === 2) return '#fa8c16';
    if (level === 1) return '#fadb14';
    return '#4096ff';
  };
  ...
  const deviationColumns: ColumnsType<DeviationGetResponse> = [
    ...
    { title: 'Параметр', dataIndex: 'type', width: 120 },
    {
      title: 'Факт',
      dataIndex: 'value',
      render: (v: number, record) => `${v} (норма: ${record.reference})`,
    },
    {
      title: 'Уровень',
      dataIndex: 'level',
      ...
      render: (lvl: DeviationGetResponse['level']) => {
        if (!lvl) return <Tag>нет данных</Tag>;
        const color = lvl === 3 ? 'red' : lvl === 2 ? 'orange' : 'gold';
        const text = lvl === 3 ? 'Критичный' : lvl === 2 ? 'Средний' : 'Низкий';
        return <Tag color={color}>{text}</Tag>;
      },
    },
    { title: 'Регламент', dataIndex: 'regulation', ellipsis: true },
    { title: 'Рекомендация', dataIndex: 'recommendation', ellipsis: true },
  ];

  const logColumns: ColumnsType<LogsGetResponse> = [
    { title: 'ID журнала', dataIndex: 'id', width: 120 },
    {
      title: 'Время',
      dataIndex: 'timestamp',
